...
I was going to have exams right after this CTF but what are exams man? (no idea, still stoning in class and in school. Brain is in NOP sled ...). So anyways I played with HATS SG and we got <insert_position>.  Also its been a long time since I last did a writeup and I kinda missed doing writeups so maybe we'll have more writeups in the future? Who knows, and anyways exams are slowly becoming a reality for me.... 
...
* Grep the kernel and linux distro/version. `GNU/Linux 5.4.0-42-generic x86_64`
* Make a VM similar to the machine in the mem-dump. [ISO](http://old-releases.ubuntu.com/releases/20.04.1/ubuntu-20.04.1-desktop-amd64.iso)
* use volatility's tool to extract `module.dwarf` and then zip it together with `/boot/System.map` to get the profile
...
(FYI, the inserting the lime kernel module is needed to extract the memory dump using LIME) 
...
Now I had to ensure that i was not tunnel visioning as it was at that time nearly 2am in the morning or something. So i did a few checks to make sure I wasn't stoning and this was the way to proceed
...
Things to note, `somethingspecial`was not loaded with any parameters (this may help in the RE process later on)
...
"Ah now you won't forget" is printed by the kernel module and should aid in the RE process. 

After dumping the module, I realised 2 things. It's a 18mb rust binary. This is way above my paygrade and its impossible for me to RE this without help. Thankfully, my 大老板 (big boss) [Daniao](https://daniao.ws/) came to help me. What followed was 1h of waiting for Ghidra and 10 minutes of looking around.
...
The last step is to of course xor the interesting labels and obtain the flag which Daniao helped do.
...
As with all things, there is always the other way of solving it. Which in this case is some "dynamic analysis" or rather replication of what the author did (to presumably check that the flag could be found)
...
So since we had to make a similar machine to extract the volatility profile, we can simply repeat the above commands to get the flag. 
